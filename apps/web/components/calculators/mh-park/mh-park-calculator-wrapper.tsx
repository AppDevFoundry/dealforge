...
import type { Deal, MhParkCalculatorInputs } from '@dealforge/types';
...
export function MhParkCalculatorWrapper() {
  ...
  if (deal && deal.type !== 'mh_park') {
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        <p className="text-destructive">This deal is not an MH Park analysis.</p>
        <button
          type="button"
          onClick={() => router.push('/mh-parks/calculator')}
          className="text-primary underline"
        >
          Start a new MH Park analysis
        </button>
      </div>
    );
  }
  ...
  return (
    <MhParkCalculator
      ...
      initialDeal={deal as Deal & { inputs: MhParkCalculatorInputs }}
